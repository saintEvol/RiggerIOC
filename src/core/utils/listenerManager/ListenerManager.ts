...
module riggerIOC {
	export class ListenerManager {
		...
		public on(caller: any, method: Function, args?: any[], once: boolean = false, coverBefore: boolean = true): Handler {
			if (!this.handlers) this.handlers = [];
			if (coverBefore) {
				// 是否有以前的
				let handler: Handler = this.handlers.find((h: Handler) => h.canReplace(caller, method));
				if(handler){
					handler.replace(caller, method, args, once);
				}
				else{
					handler = Handler.create(caller, method, args, once);
